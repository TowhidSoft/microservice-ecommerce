...
const addToCart = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!cartSessionId) {
            ...
            await redis.setex(`sessions:${cartSessionId}`, config.cartTTL, cartSessionId);
        ...
        const { data } = await axios.get(`${config.inventoryServiceUrl}/inventories/${parsedBody.data.inventoryId}`);
        if (Number(data.quantity) < parsedBody.data.quantity) {
            return res.status(400).json({ message: 'Inventory not available' })
        }

        // add item to the cart
        // TODO: Check if the product is already exists in the cart
        ...
        await axios.put(`${config.inventoryServiceUrl}/inventories/${parsedBody.data.inventoryId}`, {
            quantity: parsedBody.data.quantity,
            actionType: 'OUT',
        });
